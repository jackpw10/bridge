...
export function ResultPage() {
  ...
  // ----- compute filtered notification requirements for sending facility -----
  const sending = t.sendingFacility;
  const filteredNotifReqs = useMemo(() => {
    if (!sending) return [];
    return sending.notificationRequirements.filter((nr) => {
      // verKey gate
      if (t.verKey === 'llto' && !nr.llto) return false;
      if (t.verKey === 'hloc' && !nr.hloc) return false;
      // service-overlap gate
      if (nr.svcIds.length === 0) return true;
      return t.context.svcIds.some((svc) => nr.svcIds.includes(svc));
    });
  }, [sending, t.verKey, t.context.svcIds]);
  ...
  // ----- case summary text -----
  const summary = useMemo(() => buildSummary(), [t, facilities, specialty, diagnoses, processSteps, filteredNotifReqs]);

  function buildSummary(): string {
    const lines: string[] = [];
    lines.push('IFT Triage Result');
    lines.push(`Version: ${t.verKey.toUpperCase()}${t.acStates['ptn'] === 'Yes' ? ' (Outside PTN)' : ''}`);
    lines.push('');
    lines.push(`Sending: ${t.sendingFacility?.name ?? '—'}`);
    if (t.destFacility) lines.push(`Receiving: ${t.destFacility.name}`);

    if (t.context.diagnoses.length) {
      const names = t.context.diagnoses
        .map((id) => diagnoses.find((d) => d.id === id)?.text ?? id)
        .join(', ');
      lines.push(`Diagnoses: ${names}`);
    }
    ...
    if (filteredNotifReqs.length) {
      lines.push('');
      lines.push('Sending-facility notifications:');
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredNotifReqs.length > 0 && (
        <Card
          title="Sending-facility notification requirements"
          description={t.sendingFacility?.name}
        >
      ...
      <Card title="Reference cards" description="Quick lookup">
        ...
        {refSelected && (
          <div className="mt-3 p-3 border border-slate-200 rounded-md bg-slate-50">
            ...
            {refSelected.code && <div className="text-xs text-slate-500">code: {refSelected.code}</div>}
            {refSelected.body && <p className="text-sm mt-2 whitespace-pre-wrap">{refSelected.body}</p>}
            {refSelected.steps.length > 0 && (
              <ol className="list-decimal pl-5 text-sm mt-2 space-y-1">
                {refSelected.steps.map((s) => <li key={s.id}>{s.text}</li>)}
              </ol>
            )}
          </div>
        )}
      ...
      <Card title="Case summary" actions={<Button size="sm" variant="secondary" onClick={copy}>{copied ? 'Copied!' : 'Copy'}</Button>}>
        <pre className="text-xs whitespace-pre-wrap bg-slate-50 border border-slate-200 rounded p-3">{summary}</pre>
      </Card>

      <div className="flex justify-between">
        <Button variant="secondary" onClick={() => { t.goToWorkflow(); nav('/triage'); }}>Back</Button>
